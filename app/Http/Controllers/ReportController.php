...
class ReportController extends Controller implements HasMiddleware
{
    ...
    function feePendingFetch(Request $request)
    {
        ...
        $months = Month::orderBy('id')->pluck('name', 'id');
        $years = Year::pluck('name', 'name');
        $fee = StudentBatch::leftJoin('batches as b', 'student_batches.batch_id', 'b.id')->leftJoin('students as s', 's.id', 'student_batches.student_id')->where('date_of_admission', '<=', Carbon::createFromFormat('Y-m-d', $request->year . '-' . $request->month . '-' . Carbon::now()->month(intval($request->month))->daysInMonth))->selectRaw("student_batches.student_id, b.monthly_fee as fees, s.date_of_admission")->whereNull('student_batches.deleted_at')->where('student_batches.batch_id', $request->batch)->whereNotIn('student_batches.student_id', function ($q) use ($request) {
