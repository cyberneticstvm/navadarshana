...
class StudentAreaController extends Controller implements HasMiddleware
{
    ...
    {
        $notes = Note::whereIn('topic_id', CourseTopic::whereIn('course_id', Batch::whereIn('id', StudentBatch::where('student_id', Auth::user()->id)->pluck('batch_id'))->pluck('course_id'))->pluck('topic_id'));
        dd($notes);
        die;
    }
}
