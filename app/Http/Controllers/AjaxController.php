...
class AjaxController extends Controller implements HasMiddleware
{
    ...
    function getStudentDetails(String $studentId)
    {
        ...
        if ($student):
            ...
            $op .= "<div class='table-responsive ms-2' style='width:100%'><table class='display table'><thead><tr><th>Batch</th><th>Category</th><th>Amount</th><th>Date</th><th>Month/Year</th></tr></thead><tbody>";
    ...
    function validateFee(Request $request)
    {
        ...
        if ($request->fid == 0):
            $fee = Fee::where('student_id', decrypt($request->student_id))->where('batch_id', $request->batch_id)->where('month', $request->month)->where('year', $request->year)->where('category', 'monthly')->first();
            if ($fee->isNotEmpty()):
                return response()->json([
                    "type" => "warning",
